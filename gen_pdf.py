...
def gen(LANG):
    ...
    def on_page(canv, doc):
        ...
        canv.setFillColor(INK2); canv.drawCentredString(A4[0] / 2, 8 * mm, L(f"Page {doc.page}  ·  données oncehumandb.com / once-human.fandom.com / wikily.gg  ·  v1.4 — 5 sept. 2026", f"Page {doc.page}  ·  data oncehumandb.com / once-human.fandom.com / wikily.gg  ·  v1.4 — Sep 5, 2026"))
        canv.restoreState()
    ...
    st += [Spacer(1, 70 * mm), Paragraph("ONCE HUMAN", S["cover1"]), Paragraph(L("Guide de référence des builds", "Build reference guide"), S["cover2"]), Spacer(1, 30 * mm),
           ...
           Spacer(1, 4 * mm), Paragraph(L("Version 1.4 — 5 septembre 2026 — compagnon papier de build_planner.html et Once_Human_Builds_FR.xlsx", "Version 1.4 — September 5, 2026 — paper companion of build_planner.html and Once_Human_Builds_EN.xlsx"), S["cover3"])]
    ...
    st.append(Paragraph(L("Trois builds PvE. Le premier est le plus destructeur ; le second sert contre les boss immunisés aux statuts (Forsaken Giant) ; le troisième est un tank de soutien pour laisser les coéquipiers faire les dégâts. Ils sont préchargés dans l'onglet « Mon build » du planificateur et dans l'onglet Build de l'Excel.",
                           "Three PvE builds. The first is the most destructive; the second is for status-immune bosses (Forsaken Giant); the third is a support tank that lets teammates deal the damage. Both are preloaded in the planner's « My build » tab and in the Excel Build sheet."), S["muted"]))
    ...
    NOTES = {
     ...
     2: L("Rôle : encaisser et rendre l'équipe plus forte. Recharger la DB12 pose la zone Fortress Warfare : +30% de réduction de dégâts et +30% crit pour toi, +20% dég d'arme pour les alliés dedans (United We Stand : jusqu'à +40% selon le nombre de joueurs). Leather Boots te soigne 2,5% PV/s dans la zone (5% sous 50% PV). Savior ×4 : bouclier à chaque coup reçu, +10% dég sous bouclier, soin automatique sous 30% PV. Festering Gel pose un abri qui bloque les tirs et soigne 6% PV/s aux alliés. Tous les mods en <Survival> (+PV). Overrides : Deviant Energy Defense, Resilience, Shield Protection, Dexterity, Rapid Aid, Agility. Calibration : PV max, réduction de dégâts.",
           "Role: soak damage and make the team stronger. Reloading the DB12 drops the Fortress Warfare zone: +30% DMG reduction and +30% crit for you, +20% Weapon DMG for allies inside (United We Stand: up to +40% with more players). Leather Boots heal you 2.5% HP/s in the zone (5% under 50% HP). Savior ×4: shield on every hit taken, +10% DMG while shielded, auto-heal under 30% HP. Festering Gel builds a shelter that blocks shots and heals allies 6% HP/s. All mods in <Survival> (+HP). Overrides: Deviant Energy Defense, Resilience, Shield Protection, Dexterity, Rapid Aid, Agility. Calibration: Max HP, DMG reduction."),
